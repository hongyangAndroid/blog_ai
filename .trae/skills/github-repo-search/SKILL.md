...
> **⚠️ 重要提示**：按名称匹配时必须使用 GitHub API，不要 WebSearch。WebSearch 不支持 `in:name` 语法，会遗漏精确匹配的项目（如 `jamiepine/voicebox`）。

### 【第一优先级】直接使用 GitHub API（必须执行）
...
1. **直接 URL 构造尝试**：
   - 尝试访问 `https://github.com/{主题}` —— 检查是否存在与主题同名的仓库
   - **示例**：主题"voicebox" → `https://github.com/voicebox`（不存在）→ 但 `https://github.com/jamiepine/voicebox` 存在

**如果用户提供了具体仓库路径（如 `owner/repo`）**：
- 直接访问 `https://github.com/{owner}/{repo}`
- 将该仓库作为第一结果输出

**硬性检查点（模型自检）**：
- [ ] **是否已使用 GitHub API 执行 `in:name` 精确匹配搜索？**（必须首先完成）
- [ ] API 返回的结果中是否包含 `xxx/{主题}` 格式的仓库？
- [ ] 是否已将名称完全匹配的项目优先记录到候选池顶部？

**⚠️ 常见错误（必须避免）**：
- ❌ **使用 WebSearch 进行名称精确匹配**（WebSearch 不支持 `in:name` 语法，会遗漏如 `jamiepine/voicebox` 这样的项目）
- ❌ 并行执行主题搜索和同义词搜索（必须先完成名称匹配）
- ❌ 看到 API 结果不相关就放弃，未检查所有返回项目
- ❌ 错误使用 `in:name` 等 GitHub 专用语法在 WebSearch 中

**项目名匹配优先输出规则**：
- 如果仓库名与主题完全匹配（如主题"voicebox"匹配到 `jamiepine/voicebox`），则**必须优先输出到结果列表顶部**
- 完全匹配的项目不受 stars 门槛限制（即使 stars < 100 也应列出，但需标注）
- 其他流程（过滤、排序、分类）保持不变

**第二优先级 - 关键词组合**：
4. 核心词：用户目标词
5. 同义词：替代表达（如 long-term memory / stateful memory）
6. 场景词：coding、mcp、tool、platform、awesome、curated
7. 技术词：agent、sdk、framework、database、os
8. 排除思路：不在 query 里硬写过多负例，放到后续过滤阶段
...
**重要**：
1. **必须首先使用 GitHub API 执行 in:name 精确匹配**，这是发现精确匹配项目的唯一可靠方式
2. 禁止在名称匹配阶段使用 WebSearch 替代 GitHub API
3. 禁止一次性并行执行所有搜索
...
1. **第一优先级使用 GitHub API 进行名称匹配**（必须首先完成）：
   - 使用 `curl` 调用 GitHub Search API 进行 `in:name` 精确匹配
   - 命令示例：`curl -s "https://api.github.com/search/repositories?q={主题}+in:name&sort=stars&order=desc&per_page=50"`
   - 解析 JSON 响应，提取仓库信息

2. **第二优先级使用 WebSearch 进行关键词组合搜索**（名称匹配完成后执行）：
   - 每组 WebSearch query 都执行检索（建议每组 30-50 条）
   - 仔细分析搜索结果中的仓库链接，提取 `owner/repo` 格式
...
1. **是否优先执行了项目名直接匹配**（第一优先级不可跳过）
2. 是否完成需求对齐并明确"目标形态"
3. 是否有 5-10 组 query 且每组有目的
4. 是否记录了检索时间与配额状态
5. 是否执行了去重、硬过滤和噪音剔除
6. 是否完成仓库归属类型分类
7. 是否每个推荐都有详细项目介绍（不是一句话）
8. 是否使用固定表格字段交付
9. 是否避免把安装实施混入本流程
